...
class WandbLogger:
    ...
    def __init__(self, opt):
        ...
        if self.config.get('log_eval', None):
            self.eval_table = self._wandb.Table(columns=['esti_image', 
                                                         'noisy_image', 
                                                         'gt_image',
                                                         'psnr',
                                                         'ssim'])
        ...
        if self.config.get('log_infer', None):
            self.infer_table = self._wandb.Table(columns=['esti_image', 
                                                         'noisy_image', 
                                                         'gt_image'])
        else:
    ...
    def log_eval_data(self, cond_img, gt_img, psnr=None, ssim=None):
        ...
        if psnr is not None and ssim is not None:
            self.eval_table.add_data(
                self._wandb.Image(cond_img),
                self._wandb.Image(gt_img),
                psnr,
                ssim
            )
        else:
